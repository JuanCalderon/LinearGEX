...
def threshold_header():
    ...
    if lsvc_target.coef_.ndim == 1:  # Binary classification
        kept_tokens_count = np.count_nonzero(lsvc_target.coef_)
    else:  # Multiclass classification
        kept_tokens_count = np.count_nonzero(np.sum(np.abs(lsvc_target.coef_), axis=0))

    print(f"len(lsvc_target.coef_")

    # extras
    sparse_coef = csr_matrix(lsvc_target.coef_)
    rows, cols = sparse_coef.get_shape()
    total_elements = rows * cols
    density_sparse = f"{(sparse_coef.nnz / total_elements) * 100:.2f}%"
    variance = np.var(sparse_coef.data)
...
def hex_to_rgb(hex_color):
    """Converts a color in #RRGGBB format to an RGB tuple"""
    hex_color = hex_color.lstrip('#')
    return tuple(int(hex_color[i:i+2], 16) for i in (0, 2, 4))

def distance_color(c1, c2):
    """Euclidean distance between two RGB colors"""
    return sum((a - b) ** 2 for a, b in zip(c1, c2)) ** 0.5

def select_bg_color(color_hex):
    """
    Takes a color in hex and returns '#EBFAFF' (light blue)
    or '#FFEBEE' (light red), depending on which one is closer.
    """
    light_blue = "#EBFAFF"
    light_red = "#FFEBEE"

    rgb = hex_to_rgb(color_hex)
    d_blue = distance_color(rgb, hex_to_rgb(light_blue))
    d_red = distance_color(rgb, hex_to_rgb(light_red))

    return light_blue if d_blue < d_red else light_red
...
def htmlizer():
    ...
    for idx, row in app_data['X_test'].iterrows():
        ...
        if abs(distance) >= app_data['min_decision_value']:
            ...
            if pred_klass < 0.00:
                bg_color_tool_tip = '#EBFAFF'  # 'hsl(120, 100.00%, 94.26%)'
            else:
                bg_color_tool_tip = '#FFEBEE'
            ...
            for word, (weight, color) in unique_words_weights.items():
                ...
                if abs(weight) >= app_data['min_weight_absolute_threshold']:
                    ...
                    if weight < 0.00:
                        bg_color_weight = '#EBFAFF'  # 'hsl(120, 100.00%, 94.26%)'
                    else:
                        bg_color_weight = '#FFEBEE'

                    frag_tip = f"<table class='tooltip_text' style='background-color: {bg_color_weight};'> "
            ...
            span_tip = f"<table class='tooltip_text' style='background-color: {select_bg_color(decision_color_html)};'> "
...
def generate_chunks():
    ...
    time_per_instance_1000 = elapsed_time / 1000 if total_row > 0 else 0

    tres = 1000 * elapsed_time / total_row if total_row > 0 else 0

    timing_info = f"Time per instance: {time_per_instance:.5f} sec in {total_row} instances." # {time_per_instance_1000} / {elapsed_time} / {tres}"
...
@app.route('/get_coefficient_summary', methods=['GET'])
def get_coefficient_summary():
    """
    Provides a summary string describing the lsvc_target model's coefficient distribution,
    based on the thresholds applied in `threshold_header`.
    """
    global app_data

    absolute_threshold = app_data.get('min_weight_absolute_threshold')
    kept_tokens = app_data.get('kept_tokens_count')
    total_tokens = app_data.get('total_potential_tokens')
    # This is the P-value used for np.percentile(original_coeffs, P)
    # e.g., if P=90, threshold is 90th percentile, and kept tokens are top 10%
    percentile_used_for_threshold = app_data.get('min_weight_percentage')

    if None in [absolute_threshold, kept_tokens, total_tokens, percentile_used_for_threshold]:
        return jsonify({
            'error': 'Model data not fully initialized. Ensure build_header and threshold_header have been called.'
        }), 400

    # Percentage of tokens that are above the threshold (the "top X%")
    percentage_above_threshold = 100.0 - float(percentile_used_for_threshold)

    # Ensure values are appropriate for formatting
    # kept_tokens and total_tokens should be integers
    # absolute_threshold and percentage_above_threshold can be floats

    summary_str = (
        f"Absolute weight threshold {float(absolute_threshold):.4f}... "
        f"el {percentage_above_threshold:.0f}% que rebasa el threshold son {int(kept_tokens)} "
        f"de {int(total_tokens)} tokens (q-grams) en total"
    )

    print(jsonify({'summary_message': summary_str}))

    return jsonify({'summary_message': summary_str})
